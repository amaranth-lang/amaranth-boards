...
def DirectUSBResource(*args, dp, dn, pullup=None, vbus_valid=None,
    conn=None, attrs=None):

    io = []
    io.append(Subsignal("d_p", Pins(dp, dir="io", conn=conn, assert_width=1)))
    io.append(Subsignal("d_n", Pins(dn, dir="io", conn=conn, assert_width=1)))
    if pullup:
...
def ULPIResource(*args, data, clk, dir, nxt, stp, rst=None,
    ...
    io.append(Subsignal("clk", Pins(clk, dir=clock_dir, conn=conn, assert_width=1)))
